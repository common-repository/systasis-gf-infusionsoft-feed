...
class Infusionsoft_App{
	...
	public function initApiKeyClient($apiKey = null, $initAsFallback = false){
        ...
        $this->client = new systasis_xmlrpc_client('/crm/xmlrpc/v1', 'api.infusionsoft.com', 443);
        $GLOBALS['_systasis']['apikey'] = $this->getApiKey();
	    if ($initAsFallback){
	...
	public function sendWithoutAddingKey($method, $args, $retry = false){
        ...
        foreach($args as $argument){
            $encoded_arguments[] = systasis_php_xmlrpc_encode($argument, array('auto_dates'));
        }

        $call = new systasis_xmlrpcmsg($method, $encoded_arguments);
        ...
        if (!$req->faultCode()){
            $result = systasis_php_xmlrpc_decode($req->value());
        } else {
            $result = array();
        }
